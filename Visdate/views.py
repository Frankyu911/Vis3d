...
def calculateUpload(request):
    """
    Upload function of Calculate mode
    """
    context_dict = {}
    try:
        if request.method == "POST":
            ...
            # Use calculation scripts to return results
...
def save(request):
    """
    Save function of Accurate mode to save it to local computer
    """
    try:
        context_dict = {}
...
def update(request):
    """
    Update function of Accurate mode to change visualisation image
    """
    try:
        filename = request.GET.get("filename")
        value = request.GET.get("value")
